...
p { "At [YAPC::NA 2012](http://yapcna.org/2012), my friend [John Anderson](http://genehack.org/) gave a talk [Code Fast, Die Young, Throw Structured Exceptions](http://www.youtube.com/watch?v=pCTEdV49p1o) [(slides)](http://www.slideshare.net/genehackdotorg/code-fast-die-young-throw-structured-exceptions-13313567). During the Q&A portion of that talk I stood up and asked:" };
...
p { "The answer was _patches!_ of course. So at YAPC, John and I started working it out to see how much work it would be. Turns out there about two hundred places where Moose throws exceptions. These errors cover everything from type errors to passing no roles to `with`. And the code to support it is quite ugly. It is littered with comments like:" };
...
p { "Moose already has a pluggable error system, but it is every bit as terrible as the above comments suggest. So while John started by adding a structured exception plugin to the existing system, I went with ripping out that system and replacing it with the solid foundation of structured exceptions. New plugins can be added on top of that to stringify exceptions as the user wishes, so we do not lose anything except a lot of cruddy code." };

p { "Another reason to replace the system wholesale is that Moose leads by example. We want people to use structured exceptions for the same reasons that we want people to use objects instead of more simplistic methods." };
